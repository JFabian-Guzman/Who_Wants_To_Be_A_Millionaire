...
class Questions(State):
  def __init__(self, event_manager, file_manager):
    ...
    self.pagination_boxes = []
    for i in range(9):
      self.pagination_boxes.append(PaginationBox(((WINDOW_WIDTH / 2 - 200) + (50 * i), (WINDOW_HEIGHT / 2 + 260)), str(i + 1))) 
      self.interactive_elements.append(self.pagination_boxes[i])
  ...
  def draw(self):
    ...
    for pagination_box in self.pagination_boxes:
      pagination_box.draw()
